...
| `bri.yaml` | Basisregistratie Inkomen (authentiek inkomensgegeven) |
| `bri-plus.yaml` | Belastingdienst — uitgebreide levering (aanslag, loonketen, toeslagen, renseignering) |
| `via.yaml` | Vooringevulde aangifte inkomstenbelasting |
...
een gematerialiseerde inverse, zodat de navigatie ook terug kan lopen.
Met `inverse: false` in `relatieAfhandeling` blijft die inverse
achterwege — voor een bron die het objecttype uitsluitend via een ander
pad wil aanbieden. In `bri-plus.yaml` hangen de aangifterubrieken onder
de aanslag; zonder die onderdrukking zou `IngeschrevenPersoon`
daarnaast een tweede pad naar dezelfde gegevens openen.
...
```graphql
heeftAangifte(belastingjaar: [Int!]): [Aangifte!]
```
...
query AangiftenPerJaar($bsn: BSN!, $jaren: [Int!]) {
  ingeschrevenPersoon(bsn: $bsn) {
    heeftAangifte(belastingjaar: $jaren) {
      belastingjaar
      status
    }
  }
